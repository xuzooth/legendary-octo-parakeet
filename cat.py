...
class Cat:
    ...
    def set_texture(self):
        path = os.path.join(f'assets/images/cat{self.texture_num}.png')
        self.texture = pygame.image.load(path)
        self.texture = pygame.transform.scale(self.texture, (self.width, self.height))
...
class Trash:
    ...
    def set_texture(self):
        path = os.path.join('assets/images/trash.png')
        self.texture = pygame.image.load(path)
        self.texture = pygame.transform.scale(self.texture, (self.width, self.height))
...
class Game:
    ...
    def spawn_trash(self):
        ...
        if len(self.obstacles) > 0:
            prev_trash = self.obstacles[-1]
            x = random.randint(prev_trash.x + self.cat.width + 84, WIDTH + prev_trash.x + self.cat.width + 84)

        # empty list
        else:
            x = random.randint(WIDTH + 100, 1000)

        # create the new trash
        trash = Trash(x)
        self.obstacles.append(trash)
...
def main():
    ...
    while True:
        ...
        if game.playing:
            ...
            for trash in game.obstacles:
                trash.update(-game.speed)
                trash.show()

                # collision
                if game.collision.between(cat, trash):
                    over = True
